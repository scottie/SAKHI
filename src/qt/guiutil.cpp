...
{
    // Convert sakhi:// to sakhi:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("sakhi://"))
    {
        uri.replace(0, 12, "sakhi:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Sakhi.lnk";
}
...
{
    return GetAutostartDir() / "Sakhi.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Sakhi-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Sakhi-qt [" + tr("command-line options") + "]                     " + "\n";
